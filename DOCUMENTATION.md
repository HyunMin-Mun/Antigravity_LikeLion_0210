...
- **실행 명령**: `npm run dev`
- **특징**: Vite의 Hot Module Replacement(HMR) 기술을 통해 코드 수정 시 즉각적인 UI 반영이 지원됩니다.

### 2. 배포 및 외부 접속 (향후 계획)
- **예정 플랫폼**: GitHub Pages, Vercel 또는 Netlify
- **안내**: 현재는 로컬 환경에서 최적화되어 동작하며, 온라인 배포 시 GitHub Actions를 통한 CI/CD 파이프라인을 구축할 예정입니다.
